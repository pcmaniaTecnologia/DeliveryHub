        ...
        <CardContent className="pt-4">
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              variant="outline"
              size="sm"
              className={cn("gap-2 border-red-300 text-red-600 hover:bg-red-50 hover:text-red-700 hover:border-red-400 font-semibold", currentCashSession ? "sm:ml-auto" : "")}
            ...
            {!currentCashSession && (
              <Link href="/dashboard/cashier" className="sm:ml-auto">
        ...
        <Card className="col-span-4">
            <CardHeader className="flex flex-row items-center justify-between">
                <div>
                    ...
                    <CardDescription>Últimas movimentações de vendas.</CardDescription>
                </div>
        ...
        <Card className="col-span-4 lg:col-span-3">
            <CardHeader className="flex flex-row items-center justify-between">
                <div>
                    ...
                    <CardDescription>Top 5 produtos {dateRangeLabel}.</CardDescription>
                </div>
                        ...
                        <TableBody>
                            {topProducts.map((p, idx) => (
                                <TableRow key={idx}>
                                    ...
                                    <TableCell className="text-right font-bold text-xs py-2">{p.quantity}</TableCell>
                                </TableRow>
                            ))}
                            {topProducts.length === 0 && (
                                <TableRow>
                                    <TableCell colSpan={2} className="text-center py-6 text-muted-foreground italic text-xs">
        ...
        <Card className="border-t-4 border-t-destructive shadow-md">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <CardTitle className="text-lg text-destructive">Estoque Crítico (Atenção!)</CardTitle>
                        <CardDescription>Produtos com estoque zerado ou negativo que precisam de reposição.</CardDescription>
                    </div>
                </div>
            ...
            <CardContent>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-2">
                    {lowStockProducts.map(p => (
                        <div key={p.id} className="flex items-center justify-between p-3 rounded-lg border bg-muted/30">
                            <div>
                                ...
                                <p className="text-[10px] text-muted-foreground uppercase">{p.categoryName}</p>
                            </div>
                    ...
                    {products && products.filter(p => p.stockControlEnabled && (Number(p.stock) || 0) <= 0).length > 5 && (
                        <div className="flex items-center justify-center p-3 rounded-lg border border-dashed text-muted-foreground text-xs italic">
                            E mais {products.filter(p => p.stockControlEnabled && (Number(p.stock) || 0) <= 0).length - 5} produtos...
                        </div>
                    )}
                </div>
            </CardContent>
