  ...
  const handlePlaceOrder = async () => {
    ...
    try {
        ...
        const orderData = {
            ...
            ...(isTableMode ? { waiterName: String(waiterNameFromUrl || 'Autoatendimento') } : {}),
            deliveryFee: Number(deliveryFee) || 0,
            paymentMethod: String(isTableMode ? 'A Combinar' : (selectedPayment === 'Dinheiro' && cashAmount ? `Dinheiro (Troco para R$${parseFloat(cashAmount.replace(',', '.')).toFixed(2)})` : (selectedPayment || 'A Combinar'))),
        ...
        if (stockItems.length > 0) {
            ...
            }).catch(err => console.error("Erro ao chamar api de estoque:", err));
        }
        // ─────────────────────────────────────────────────────────────────
        ...
        if (companyData.phone) {
            ...
            if (zapNumber) {
                const whatsappUrl = `https://wa.me/${zapNumber}?text=${encodeURIComponent(message)}`;
